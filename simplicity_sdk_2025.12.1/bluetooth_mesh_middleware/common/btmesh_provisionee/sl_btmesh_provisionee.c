...
#ifdef SL_CATALOG_BTMESH_CMT_PRESENT
#include "sl_btmesh_cmt.h"
#include "sl_btmesh_cmt_config.h"

#if SL_BTMESH_CMT_USE_OOB && SL_BTMESH_PROVISIONEE_OOB_ENABLE_CFG_VAL
#warning "Both SL_BTMESH_CMT_USE_OOB and SL_BTMESH_PROVISIONEE_OOB_ENABLE_CFG_VAL are enabled. CMT takes precedence."
#endif // SL_BTMESH_CMT_USE_OOB && SL_BTMESH_PROVISIONEE_OOB_ENABLE_CFG_VAL

#endif // SL_CATALOG_BTMESH_CMT_PRESENT
...
void sl_bt_provisionee_on_event(sl_bt_msg_t* evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    #if defined(SL_CATALOG_BTMESH_CMT_PRESENT) && SL_BTMESH_CMT_USE_OOB
      log_info("Initializing with OOB data from mesh token" NL);
      uint8_t  output_size = 0;
      uint16_t output_actions = 0;
      uint8_t  input_size = 0;
      uint16_t input_actions = 0;
      sc = sl_btmesh_cmt_get_oob_data(&output_size,
                                      &output_actions,
                                      &input_size,
                                      &input_actions);
      if (sc != SL_STATUS_OK) {
        log_status_error_f(sc, "Failed to get OOB data" NL);
    ...
    #elif SL_BTMESH_PROVISIONEE_OOB_ENABLE_CFG_VAL
      log_info("Initializing with OOB data" NL);
      sc = sl_btmesh_node_init_oob(SL_BTMESH_PROVISIONEE_OOB_PUBLIC_KEY_ENABLE,
        SL_BTMESH_PROVISIONEE_AUTH_METHODS,
