 ...
 * replay protection list size) should be set according to what is
 * actually needed. That is, max_elements should be equal to the
 * number of elements present in the DCD, and so on.
 *
...
typedef struct {
  ...
  /**  Maximum number of Key Refresh requests pending for the Provisioner */
  uint8_t limit_prov_concurrent_kr;
